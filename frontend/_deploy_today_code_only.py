...
# Backup real de la BD activa y deploy del código sin tocarla
for cmd in [
    "mkdir -p /root/specialwash/backend/instance/backups && cp /root/specialwash/backend/instance/specialwash.db /root/specialwash/backend/instance/backups/specialwash-predeploy-$(date +%Y%m%d-%H%M%S).db && ls -1t /root/specialwash/backend/instance/backups | head -3",
