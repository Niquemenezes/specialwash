...
function exportarExcelCompleto({ ausenciasAnio, empleados, anio }) {
  ...
  XLSX.writeFile(wb, `SW_STUDIO_Vacaciones_${anio}.xlsx`);
}

function imprimirAusencias({ visibles, mes, anio, empleadoNombre }) {
  const titulo = `SW Studio — Vacaciones y ausencias — ${mes ? MESES[mes - 1] + " " : ""}${anio}${empleadoNombre ? " — " + empleadoNombre : ""}`;
  const filaHtml = (a) => `<tr>
...
<body>
  <h2>SW Studio — Vacaciones y ausencias</h2>
  <p>${titulo.replace("SW Studio — Vacaciones y ausencias — ", "")}</p>
  <table>
    <thead><tr>
