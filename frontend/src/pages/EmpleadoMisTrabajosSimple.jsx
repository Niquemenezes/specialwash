...
const PRIORIDAD_EMPLEADO = {
  1: { label: "Urgente",     color: "#f59e0b", bg: "#fff7ed", border: "#fed7aa" },
  2: { label: "Muy urgente", color: "#dc2626", bg: "#fef2f2", border: "#fecaca" },
};

function TrabajoCarte({ parte, onAccion, tipo = "proximo" }) {
  const pCfg = PRIORIDAD_EMPLEADO[parte.prioridad];
  ...
  return (
    <div
      ...
      style={{
        background: "white",
        border: pCfg ? `1px solid ${pCfg.border}` : `1px solid #e5e7eb`,
        borderLeft: pCfg ? `4px solid ${pCfg.color}` : `1px solid #e5e7eb`,
        borderRadius: "10px",
        padding: "16px",
        marginBottom: "12px",
        position: "relative",
          ...
          >
            ...
            {pCfg && (
              <span style={{
                ...
                borderRadius: "999px", background: pCfg.bg,
                color: pCfg.color, border: `1px solid ${pCfg.border}`,
              }}>
                {parte.prioridad === 2 ? "🔴" : "⚡"} {pCfg.label}
              </span>
            )}
          </div>
