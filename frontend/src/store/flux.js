  ...
  try {
    ...
    _loadingProductos = false;
      ...
      crearInspeccion: async (payload) => {
        try {
          const created = await apiFetch("/api/inspeccion-recepcion", { 
            method: "POST", 
            body: payload 
          });
          return created;
        } catch (err) {
          console.error("crearInspeccion:", err);
          throw err;
        }
      },
      ...
      getMisInspecciones: async () => {
        try {
          ...
          console.error("getMisInspecciones:", err);
          return [];
        }
      },
      ...
      getInspeccion: async (id) => {
        try {
          ...
          console.error("getInspeccion:", err);
          throw err;
        }
      },
      ...
      subirFotoInspeccion: async (inspeccionId, archivo) => {
        try {
          ...
          const data = await apiFetch(
            `/api/inspeccion-recepcion/${inspeccionId}/upload-foto`,
            {
              method: "POST",
              body: formData,
              json: false, // No enviar como JSON porque es FormData
              headers: {} // FormData establece su propio Content-Type
            }
          );
          return data;
        } catch (err) {
          console.error("subirFotoInspeccion:", err);
          throw err;
        }
      },
      ...
      subirVideoInspeccion: async (inspeccionId, archivo) => {
        try {
          console.log(`[API] Subiendo video: ${archivo.name} (${(archivo.size / (1024 * 1024)).toFixed(2)} MB)`);
          
          const formData = new FormData();
          formData.append("file", archivo);
          
          const data = await apiFetch(
            `/api/inspeccion-recepcion/${inspeccionId}/upload-video`,
            {
              method: "POST",
              body: formData,
              json: false, // No enviar como JSON porque es FormData
          ...
          console.log("[API] Video subido correctamente:", data);
          return data;
        } catch (err) {
          console.error("[API ERROR] Error al subir video:", {
      ...
      actualizarInspeccion: async (id, payload) => {
        try {
          const updated = await apiFetch(`/api/inspeccion-recepcion/${id}`, {
            method: "PUT",
            body: payload
          });
          return updated;
        } catch (err) {
          console.error("actualizarInspeccion:", err);
          throw err;
        }
      },
      ...
      registrarEntregaInspeccion: async (id, payload) => {
        try {
          const updated = await apiFetch(`/api/inspeccion-recepcion/${id}/entrega`, {
            method: "POST",
            body: payload
          });
          return updated;
      ...
      guardarActaInspeccion: async (id, payload) => {
        try {
          const updated = await apiFetch(`/api/inspeccion-recepcion/${id}/acta`, {
            method: "POST",
            body: payload
          });
          return updated;
        } catch (err) {
          console.error("guardarActaInspeccion:", err);
          throw err;
        }
      },
      ...
      sugerirActaInspeccion: async (id, payload) => {
        try {
          const data = await apiFetch(`/api/inspeccion-recepcion/${id}/sugerir-acta`, {
            method: "POST",
            body: payload
          });
          return data;
        } catch (err) {
          console.error("sugerirActaInspeccion:", err);
          throw err;
        }
      },
      ...
      chatActaInspeccion: async (id, payload) => {
        try {
          const data = await apiFetch(`/api/inspeccion-recepcion/${id}/chat-acta`, {
            method: "POST",
            body: payload
          });
          return data;
        } catch (err) {
          console.error("chatActaInspeccion:", err);
          throw err;
        }
      },
      ...
      eliminarInspeccion: async (id) => {
        try {
          await apiFetch(`/api/inspeccion-recepcion/${id}`, { method: "DELETE" });
          return true;
        } catch (err) {
          console.error("eliminarInspeccion:", err);
          throw err;
        }
      },
    },
